"""
Fit RI model (shared alpha/beta, per-model x) and bootstrap SE/CI for the
"set2" text-noise dataset reconstructed from aggregated counts
(see reconstruct_text_noise_set2.py -> data/text_noise_set2.csv).

Models: GPT-5.2, GPT-3.5-turbo, Gemini-2.5, Gemini-2.0
Noise levels: p' = 0.0 .. 1.0 (11 levels), 400 rows (200 Y1 + 200 Y2) each.

Method (Efron & Tibshirani, 1993):
  1. Load data/text_noise_set2.csv (long format: model, p_prime, trial_id, label, pred)
  2. Fit RI model jointly (SSE minimisation, shared alpha/beta, per-model x)
  3. For each bootstrap replicate: resample each (model, p_prime) 400-row cell
     independently with replacement, recompute Pc, refit
  4. SE = std of bootstrap dist; 95% CI = [2.5th, 97.5th percentile]

Output:
  data/results/text_noise_set2/
    bootstrap_raw.csv           -- B rows of bootstrap parameter estimates
    bootstrap_summary.csv       -- estimate, SE, 95% CI per parameter
    pairwise_lambda_tests.csv   -- pairwise lambda significance between model pairs

Usage:
    python fit_text_noise_set2.py
    python fit_text_noise_set2.py --B 200   # quick test
    python fit_text_noise_set2.py --B 1000 --seed 0
"""
...
# CONFIG
# ==============================================================================
DATA_CSV = "./data/text_noise_set2.csv"
...
# ── RI model (identical to fit_text_noise.py) ─────────────────────────────────
...
def compute_pc(label, pred):
    """Pc = P(Y2)*P(pred=Ab|Y2) + P(Y1)*P(pred=Aa|Y1)."""
    p_s2  = label.mean()
    p_s1  = 1.0 - p_s2
    mask1 = label == 1
    mask0 = label == 0
...
def load_data():
    """Returns {model: {p_prime: (label_array, pred_array)}}"""
    df = pd.read_csv(DATA_CSV)
    all_data = {}
    for model in MODELS:
        sub = df[df["model"] == model]
        levels = {}
        for p, g in sub.groupby("p_prime"):
            levels[float(p)] = (g["label"].values.astype(int), g["pred"].values.astype(int))
        all_data[model] = levels
    return all_data


def build_datasets(all_data, idx_fn=None):
    """
    Build list of (p_obs, Pc_obs) per model for joint RI fitting.
    idx_fn: optional callable(n) -> resample indices, applied independently
            to each (model, p_prime) cell (None = full data, no resampling).
    """
    datasets = []
    labels   = []
    for model in MODELS:
        if model not in all_data:
            continue
        p_vals, pc_vals = [], []
        for p, (label, pred) in sorted(all_data[model].items()):
            if idx_fn is not None:
                idx = idx_fn(len(label))
                t, pr = label[idx], pred[idx]
            else:
                t, pr = label, pred
            p_vals.append(p)
            pc_vals.append(compute_pc(t, pr))
...
def bootstrap(all_data, B, rng):
    records = []
    for b in range(B):
        idx_fn = lambda n: rng.integers(0, n, size=n)
        datasets, labels = build_datasets(all_data, idx_fn=idx_fn)
        if not datasets:
            continue
        try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Fit RI model + bootstrap SE for text-noise 'set2' dataset")
    ...
    all_data = load_data()
    for model in MODELS:
        n_levels = len(all_data.get(model, {}))
        print(f"  {model}: {n_levels} noise levels")
    ...
    print("  sig_z_0.05 / sig_ci = True  ->  reject H0: lambda_A = lambda_B")
    print("="*90)
